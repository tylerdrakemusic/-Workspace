...
EVIDENCE_POLICY = todo_decision_contract.EVIDENCE_POLICY
...
def validate_decision_metadata(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    if impact >= EVIDENCE_POLICY["high_impact_min_score"] and not evidence:
        raise DecisionMetadataError("high-impact metadata requires evidence")
    if (
        impact >= EVIDENCE_POLICY["very_high_impact_min_score"]
        and len(evidence) < EVIDENCE_POLICY["very_high_impact_min_evidence"]
    ):
