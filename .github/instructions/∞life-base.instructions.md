...
∞Life currently uses **static HTML** dashboards only (`type: "static_html"` in `dashboard.json`). If a Flask app is ever introduced, wire all four auto-start components:

1. **`f:\∞Life\dashboard.json`** — add entry with `"type": "flask_app"`, `"port": <n>`, `"url": "http://127.0.0.1:<n>"`
2. **`f:\⊕Workspace\tools\start_<appname>.ps1`** — PowerShell launcher: set `$env:PYTHONPATH = "f:\∞Life\src"`, call `C:\G\python.exe <path> --port <n>`
3. **`f:\⊕Workspace\tools\portal_servers.json`** — add entry with `name`, `port`, `project: "∞Life"`, `cmd` pointing to the `.ps1`, `enabled: true`
4. **`f:\⊕Workspace\reports\portal.html`** — add `{"port": <n>, "name": "..."}` to the `SERVERS` JS array AND add a `<div class="server-row">` status-dot in the sidebar

**Existing dashboards are static HTML** — they are generated by a CLI script and opened via `file://` iframe; they do NOT need portal_servers entries.
