...
Architecture diagram publication protocol: when the beautifier touches or migrates diagrams, it must publish deterministic HTML artifacts under `reports/diagrams/<stem>.html`, retain escaped source provenance linking each artifact to its `diagrams/<stem>.mmd` source, record proof for the mapping, and record an FR event with the publication count. The one-time migration pass is idempotent, processes every canonical `diagrams/*.mmd` source, and must preserve Mermaid semantics. The dashboard discovers and displays these published HTML artifacts as canonical output; SVGs remain compatibility fallbacks only.
