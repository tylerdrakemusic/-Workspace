...
2. **`f:\⊕Workspace\tools\start_<appname>.ps1`** — PowerShell launcher setting `PYTHONPATH` + calling the script
3. **`f:\⊕Workspace\tools\portal_servers.json`** — entry with `name`, `port`, `project: "👁AI-Manifest"`, `cmd`, `enabled: true`
4. **`f:\⊕Workspace\reports\portal.html`** — add port to `SERVERS` JS array + sidebar server-row div
